...
Form::Form()
: _name("default"), _grade_to_sign(100), _grade_to_exec(100)
{
...
_grade_to_sign(orig.getGradeRequiredToSign()),
...
Form& Form::operator=(const Form &orig)
{
	if (this != &orig)
	{
		...
		*(const_cast<std::string*>(&_name)) = orig.getName();		// _name = 
		_is_signed = orig.getIsSigned();
...
{
	if (_grade_to_sign > bure.getGrade())
